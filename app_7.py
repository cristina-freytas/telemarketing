...
def main():
    ...
    if (data_file_1 is not None):
        ...
        # Verificar se a coluna 'y' existe
        if 'y' not in bank_raw.columns:
            st.error("A coluna 'y' não foi encontrada no DataFrame.")
            return  # Se a coluna não existe, interrompe a execução
        ...
        # Calcule as proporções para 'y' em ambos os DataFrames
        ...
        if graph_type == 'Barras':
            sns.barplot(x=bank_raw_target_perc.index, 
                        y=bank_raw_target_perc.iloc[:, 0],  # Usando a primeira coluna, que é a porcentagem
                        ax=ax[0])
            ax[0].bar_label(ax[0].containers[0])
            ax[0].set_title('Dados brutos', fontweight="bold")

            sns.barplot(x=bank_target_perc.index, 
                        y=bank_target_perc.iloc[:, 0],  # Usando a primeira coluna, que é a porcentagem
                        ax=ax[1])
            ax[1].bar_label(ax[1].containers[0])
            ax[1].set_title('Dados filtrados', fontweight="bold")
        else:
            bank_raw_target_perc.plot(kind='pie', autopct='%.2f', y=bank_raw_target_perc.columns[0], ax=ax[0])  # Ajuste para garantir que 'y' seja usada corretamente
            ax[0].set_title('Dados brutos', fontweight="bold")

            bank_target_perc.plot(kind='pie', autopct='%.2f', y=bank_target_perc.columns[0], ax=ax[1])  # Ajuste para garantir que 'y' seja usada corretamente
            ax[1].set_title('Dados filtrados', fontweight="bold")
